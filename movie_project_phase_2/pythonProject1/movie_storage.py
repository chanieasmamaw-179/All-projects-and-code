...
def get_movies():
    try:
        ...
        print(Fore.RED + "Movies file not found." + Style.RESET_ALL)
        return {}
...
def add_movie(movies, title, rating, genre, year, **kwargs):
    ...
    if title_lower in movies:
        ...
        print(
            f"Added {title} ({year}) to {genre} with a rating of {rating}. Additional properties: {kwargs}"
        )
...
def delete_movie(movies, movie_title):
    ...
    best_match, score = process.extractOne(
        movie_title_lower, movie_titles, scorer=fuzz.partial_ratio
    )
